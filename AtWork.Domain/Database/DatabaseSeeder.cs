...
namespace AtWork.Domain.Database
{
    public static class DatabaseSeeder
    {
        ...
        {
            if (!context.TB_Usuario.Any()) // Exemplo para verificar se já tem dados
            {
                TB_Usuario usuario = new()
                {
                    Email = "gustavoh1709@hotmail.com",
                    Nome = "Gustavo Henrique Borges (Web)",
                    Login = "gustavo_web",
                    Senha = "MTIz",
                ...
                TB_Grupo_X_Admin grupo_x_admin = new()
                {
                    ID_Grupo = grupo.ID,
                ...
                TB_Funcionario funcionario = new()
                {
                    Email = "gustavoh1709@hotmail.com",
                    ID_Usuario = usuario.ID,
                    Nome = "Gustavo Henrique Borges (Mobile)",
                ...
                TB_Ponto ponto = new()
                {
                    ...
                    DT_Ponto = DateTime.Now.ToBrazilianTime(),
                    DT_Alt = DateTime.Now.ToBrazilianTime(),
                    DT_Cad = DateTime.Now.ToBrazilianTime(),
                    ST_Ponto = StatusPonto.PendenteAprovacao,
                    TP_Ponto = TipoPonto.Entrada,
                };

                context.TB_Ponto.Add(ponto);

                TB_Justificativa justificativa = new()
                {
                    DT_Justificativa = DateTime.Now.ToBrazilianTime(),
                    DT_Alt = DateTime.Now.ToBrazilianTime(),
                    DT_Cad = DateTime.Now.ToBrazilianTime(),
                    ST_Justificativa = StatusJustificativa.PendenteAprovacao,
                    Justificativa = "Criado Pelo Database Seeder",
                    ID_Funcionario = funcionario.ID,
                    ImagemContentType = "image/jpg",
                    ImagemJustificativa = Convert.FromBase64String(ImageB64)
                };

                context.TB_Justificativa.Add(justificativa);
